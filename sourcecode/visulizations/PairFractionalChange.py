...
width_type1 = 'passive'
width_type2 = 'passive'
home_folder = '/Users/dmanral/Desktop/Analysis/TARA/Task11/Connectivities/'
...
def plot_change(fraction, species, codes, depth1, depth2, width_type1, width_type2):
    ...
    ax.set_title("minimum: {0}%, maximum: {1}%, average: {2}%".format(round(min_fraction, 2), round(max_fraction, 2),
                                                                      np.round(avg, 2)),
                 pad=70, size=20)
    plt.suptitle("Fractional change ({0}_{1}z-{2}_{3}z)/{2}_{3}z".format(width_type2, depth2, width_type1, depth1), fontsize=20)
    ...
    divnorm = colors.TwoSlopeNorm(vmin=-50, vcenter=0, vmax=250)
    ...
    # plt.show()
    plt.savefig(
        home_folder + "{4}/t{0}m/FractionalChange_z{0}{1}_z{2}{3}_{5}_limit-50-250_g.png".format(depth2, width_type2, depth1,
