...
data = np.load('/Users/dmanral/Desktop/Analysis/TARA/Task4/Full_connectivity_output/' + 'Stations_min-T_connectivity_nan.npz', allow_pickle=True)
codes = data['codes']
...
plt.margins(0, 0)
ax = plt.gca()
ax.set_title("Fractional change Average: {2}%".format(depth2, depth1, np.round(avg, 2)),
             pad=70)
plt.suptitle(
    "Fractional change in minimum connectivity time between all stations ")
...
plt.imshow(fraction, cmap=plt.cm.bwr, norm=divnorm)

cbar = plt.colorbar(orientation='vertical')

cbar.set_label('Fractional change (%)')
# plt.show()
plt.savefig(output_folder + "1FractionalChange_z{0}m_vs_z{1}m_{2}.pdf".format(depth1, depth2,species), bbox_inches='tight',
            pad_inches=0.2)
