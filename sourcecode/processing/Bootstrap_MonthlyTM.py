...
def compute_transition_matrix(mon, hex_indices, map_h3_to_mat, no_grids, sim_depth, output_path, no_particles, sample_set):
    t_mon1 = time()
    ...
    for file in files:
        ...
        if len(sample_set) < no_particles:
            invalid_indices = np.union1d(invalid_indices, np.delete(range(no_particles), sample_set))
        delete_count +=  len(invalid_indices) + len(sample_set) - no_particles
...
def main():
    ...
    hex_indices = np.append(master_uni_hex, (NEW))
    ...
    for size in sample_size:
        ...
        for state in np.arange(init_state, end_state + 1):
            ...
            path_dir = data_folder + 'Boot_Sample/Size_{0}/Monthly/State_{1}/'.format(size, state)

            os.makedirs(path_dir, exist_ok=True)
            [compute_transition_matrix(mon, hex_indices, map_h3_to_mat, no_grids, sim_depth, path_dir, no_particles,
